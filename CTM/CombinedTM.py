...
class CombinedTM(nn.Module):
    def __init__(self, vocab_size, contextual_embed_size, num_topics=50, en_units=200, dropout=0.4,
                 positive_sampling='random', negative_sampling='random'):
        super().__init__()

        self.vocab_size = vocab_size
        self.num_topics = num_topics
        self.contextual_embed_size = contextual_embed_size
    ...
    def get_theta(self, x):
        # x is the concatenated tensor of bow and contextual_emb
        bow = x[:, :self.vocab_size]
        contextual_emb = x[:, self.vocab_size:]
        
        projected_contextual = self.fc_contextual(contextual_emb)
        combined_input = torch.cat((bow, projected_contextual), dim=1)
    ...
    def forward(self, indices, input, epoch_id=None):
        ...
        # Concatenate bow and contextual_emb to create the input for get_theta
