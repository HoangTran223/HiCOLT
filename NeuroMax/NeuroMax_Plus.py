...
class NeuroMax_Plus(NeuroMax):
    def __init__(self, weight_loss_CLT=1.0, weight_loss_CLC=1.0, weight_loss_DTC=1.0, weight_loss_TP=1.0,
                 threshold_epoch=10, threshold_cluster=30, metric_CL='cosine', max_clusters=9,
                 alpha_TP=20.0, sinkhorn_max_iter=500, contextual_embed_size=768, vocab=None,
                 contrastive_loss_type='triplet', positive_sampling='random', negative_sampling='random', **kwargs):
        neuro_kwargs = dict(kwargs)
        for k in ['positive_sampling', 'negative_sampling', 'contrastive_loss_type']:
            neuro_kwargs.pop(k, None)
        super().__init__(**neuro_kwargs)
        self.vocab = vocab
        ...
        self.document_emb_prj = nn.Sequential(
            nn.Linear(contextual_embed_size, self.word_embeddings.shape[1]),
            nn.ReLU(),
    ...
    def get_loss_TP(self, contextual_emb, indices):
        minibatch_embeddings = contextual_emb 
    ...
    def get_loss_DTC(self, contextual_emb, theta, margin=0.2, k=5, metric='cosine', gamma=32):
    ...
    def forward(self, indices, input, epoch_id=None):
        ...
        contextual_emb = input[1].to(self.device)
        ...
        if self.group_connection_regularizer is not None and epoch_id is not None and epoch_id > 10:
        ...
        if epoch_id is not None and epoch_id >= self.threshold_epoch:
            loss_TP = self.get_loss_TP(contextual_emb, indices)
        ...
        loss = loss_TM + loss_ECR + loss_GR + loss_InfoNCE + loss_TP + loss_CLC + loss_CLT + loss_DTC
